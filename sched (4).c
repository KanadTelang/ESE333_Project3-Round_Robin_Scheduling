...
struct node* pending_head= NULL;
//tail for the arrival queue
struct node* pending_tail= NULL;
...
void printlist(struct node * list){
	printf("Entering my own print Kanad \n"); 
	struct node * temp = list; 
	while (temp!= NULL){
		printf("ID: %s, service time: %d , Arr time: %d\n", temp-> p -> process_ID, temp -> p -> service_time, temp->p-> arrival_time) ; 
		temp= temp-> next; 
	}
}
...
void read_file()
{
	...
	if (pending_head == NULL){
		...
		pending_tail = newnode; 
		newnode -> p= newproc; 
		newnode -> next= NULL; 
	}
	else{
		newnode-> next= NULL; 
		pending_tail ->next = newnode; 
		pending_tail = newnode; 
		newnode -> p= newproc; 
		newnode -> next= NULL; 
	}
...
int main()
{
	 ...
	 */
		while (pending_head != NULL || head != NULL ||computer.cores[0]. busy==1 || computer.cores[1]. busy==1 || computer.cores[2]. busy==1 || computer.cores[3].busy==1){
		...
		for (int i=0; i<4; i++){
			if(computer.cores[i].busy == 1){
				if (computer.cores[i].p->service_time == 0 || computer.cores[i].proc_time >= quantum ){// computer.cores[i].p -> io ==1 task 3 
 					remove_proc(i); 
				}
			}
				if (head!= NULL && computer.cores[i].busy==0){
					if (head == tail){
						tail= NULL; 
					}
